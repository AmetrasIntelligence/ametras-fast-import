...
_stub("models.orm_backend").OrmBackend = type(
    "OrmBackend", (), {"__init__": lambda s, *a, **kw: None}
)
...
_constants.PAUSE_POLL_INTERVAL_SECONDS = 2
_constants.MAX_POLL_ERROR_ENTRIES = 500
_constants.MAX_FINAL_ERROR_ENTRIES = 5_000
_constants.MIN_BATCH_SIZE = 10
_constants.MAX_BATCH_SIZE = 1000
_constants.DEFAULT_BATCH_SIZE = 200
_constants.DEFAULT_ENCODING = "utf-8"
_constants.DEFAULT_DELIMITER = ","
_constants.DEFAULT_RETRY_LIMIT = 3
_constants.DEFAULT_RETRY_DELAY_MS = 500
...
def _make_log(
    cancel=False, pause=False, skip_file="", log_id=1, table="csv_import_log"
):
    log = MagicMock()
    log.id = log_id
    log._table = table
        ...
        job = _make_job(
            db_rows=[
                ...
                (
                    ...
                    False,
                    "other.csv",
                ),  # _wait_if_paused: not paused, exits immediately
                (
                    False,
                    False,
                    "other.csv",
                ),  # post-wait _check_control → CONTINUE (no cancel)
            ]
        )
...
class TestFilterPendingRows(unittest.TestCase):
    ...
    def test_filters_already_processed_rows(self):
        ...
        fp = {
            "f.csv": {"processedRanges": [[1, 3]], "totalRows": 5, "failedIndices": []}
        }
    ...
    def test_empty_when_all_processed_no_failures(self):
        rows = [_FakeRow(i) for i in range(1, 4)]
        fp = {
            "f.csv": {"processedRanges": [[1, 3]], "totalRows": 3, "failedIndices": []}
        }
        self.assertEqual(self.job._filter_pending_rows("f.csv", rows, fp), [])
