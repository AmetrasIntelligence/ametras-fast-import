...
class TestParseRefs(unittest.TestCase):
    ...
    def test_real_route_ids(self):
        result = parse_refs('purchase_stock.route_warehouse0_buy,stock.route_warehouse0_mto')
        self.assertEqual(result, ['purchase_stock.route_warehouse0_buy', 'stock.route_warehouse0_mto'])
...
class TestNormalizeExtId(unittest.TestCase):
    ...
    def test_without_module(self):
        self.assertEqual(normalize_ext_id('product_category#123'), ('__import__', 'product_category#123'))

    def test_with_whitespace(self):
        self.assertEqual(normalize_ext_id(' base.de '), ('base', 'de'))

    def test_multiple_dots(self):
        self.assertEqual(normalize_ext_id('purchase_stock.route_warehouse0_buy'), ('purchase_stock', 'route_warehouse0_buy'))


class TestLookupRef(unittest.TestCase):
    def test_found(self):
        ref_map = {('product.category', '__import__', 'product_category#123'): 42}
        self.assertEqual(lookup_ref('product.category', 'product_category#123', ref_map), 42)

    def test_with_module(self):
        ref_map = {('stock.route', 'purchase_stock', 'route_warehouse0_buy'): 7}
        self.assertEqual(lookup_ref('stock.route', 'purchase_stock.route_warehouse0_buy', ref_map), 7)
...
class TestStandardDbIdModels(unittest.TestCase):
    def test_contains_expected(self):
        for model in ['res.country', 'res.currency', 'uom.uom', 'res.lang',
                       'res.country.state', 'res.partner.title']:
...
class TestPrefetchReferences(unittest.TestCase):
    ...
    def test_prefetch_many2many(self):
        ...
        backend.create('ir.model.data', {
            'module': '__import__', 'name': 'tag_c',
            'model': 'res.partner.category', 'res_id': 1,
        })
        backend.create('ir.model.data', {
            'module': '__import__', 'name': 'tag_v',
...
class TestResolveRow(unittest.TestCase):
    ...
    def test_resolve_many2one_external_id(self):
        ...
        resolved, warnings = resolve_row(
            backend, 'res.partner', field_info,
            {'name': 'Alice', 'country_id': 'base.de'}, ref_map,
        )
