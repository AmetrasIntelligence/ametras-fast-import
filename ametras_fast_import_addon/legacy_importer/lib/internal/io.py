...
def write_csv(filename, header, data, encoding="utf-8"):
    ...
    c = UnicodeWriter(
        file_result, delimiter=";", quoting=csv.QUOTE_ALL, encoding=encoding
    )
    c.writerow(header)
    for d in data:
        c.writerow(d)
...
def write_file(
    ...
    with open(launchfile, mode) as myfile:
        myfile.write(
            '%s %sodoo_import_thread.py -c %s --file=%s --model=%s --encoding=%s --worker=%s --size=%s --groupby=%s --ignore=%s --sep="%s" %s\n'
            % (
                python_exe,
                path,
                conf_file,
        ...
        if fail:
            myfile.write(
                '%s %sodoo_import_thread.py -c %s --fail --file=%s --model=%s --encoding=%s --ignore=%s --sep="%s" %s\n'
                % (
                    python_exe,
                    path,
                    conf_file,
