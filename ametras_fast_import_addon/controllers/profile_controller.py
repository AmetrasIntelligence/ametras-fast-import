...
class ProfileController(http.Controller):
    ...
    @http.route(
        "/ametras_fast_import/profile/upload",
        type="http",
        auth="user",
        methods=["POST"],
        csrf=False,
    ...
    @http.route(
        "/ametras_fast_import/profile/list", type="json", auth="user", methods=["POST"]
    )
    ...
    @http.route(
        "/ametras_fast_import/profile/<int:profile_id>",
        type="json",
        auth="user",
        methods=["POST"],
    )
    ...
    @http.route(
        "/ametras_fast_import/profile/<int:profile_id>/update",
        type="json",
        auth="user",
        methods=["POST"],
    )
    ...
    @http.route(
        "/ametras_fast_import/profile/<int:profile_id>/delete",
        type="json",
        auth="user",
        methods=["POST"],
    )
    ...
    @http.route(
        "/ametras_fast_import/profile/<int:profile_id>/export",
        type="http",
        auth="user",
        methods=["GET"],
    )
    ...
    def _parse_field_mappings_csv(self, csv_text):
        ...
        is_rich = any(h.strip().lower() in ("required", "transform") for h in header)
    ...
    def _generate_profile_zip(self, profile):
        ...
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
            ...
            zf.writestr(
                "mappings.csv", self._serialize_mappings_csv(data.get("mappings", []))
            )
            zf.writestr(
                "sequence.csv", self._serialize_sequence_csv(data.get("sequence", []))
            )
            run_settings = data.get("run_settings", {})
            if run_settings:
                zf.writestr(
                    "run_settings.csv", self._serialize_key_value_csv(run_settings)
                )
