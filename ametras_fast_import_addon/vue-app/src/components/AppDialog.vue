  ...
  <Teleport to="body">
    <Transition name="csv-dialog-fade">
      <div
        ...
        @click.self="cancel"
      >
        <div class="csv-dialog card shadow-lg" :class="`csv-dialog--${state.type}`" role="dialog" aria-modal="true" aria-labelledby="csv-dialog-title">
          <div class="csv-dialog__header">
            ...
            <span id="csv-dialog-title" class="csv-dialog__title">{{ dialogTitle }}</span>
          </div>
...
.csv-dialog-overlay {
  position: fixed;
  inset: 0;
  z-index: 1070;  /* above dropdowns (1060) and Odoo modal (1055) */
  display: flex;
  align-items: center;
  justify-content: center;
  background: var(--csv-overlay-bg, rgba(0, 0, 0, 0.5));
