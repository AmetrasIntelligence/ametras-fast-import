  ...
  <div>
    <div class="row row-cols-2 g-3">
      <div>
        <label class="form-label small text-body-secondary mb-1">
          ...
          <span v-if="isStandalone" class="text-body-secondary">({{ batchSizeMin }}-{{ batchSizeMax }})</span>
        </label>
        <input
      ...
      <div>
        <label class="form-label small text-body-secondary mb-1">
          {{ $t('settings.workers') }}
          <span v-if="!isStandalone" class="text-body-secondary">{{ $t('settings.workersRange') }}</span>
        </label>
        <input
        ...
        />
        <small v-if="isStandalone" class="d-block text-body-secondary mt-1">
          {{ $t('settings.standalone.workersFixed') }}
        </small>
        <div v-else-if="workersError" class="invalid-feedback">
          {{ workersError }}
        </div>
    ...
    <div class="row row-cols-2 g-3 mt-1">
      ...
      <div class="d-flex align-items-center gap-2 pt-4">
        ...
        />
        <label for="dryRun" class="form-check-label small" :class="{ 'text-body-secondary': isStandalone }">{{ $t('settings.dryRun') }}</label>
        <small v-if="isStandalone" class="text-body-secondary">{{ $t('settings.standalone.dryRunUnavailable') }}</small>
      </div>
      <div>
