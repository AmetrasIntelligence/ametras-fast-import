...
function formatTransform(transform: FieldTransform): string {
  switch (transform.type) {
    ...
    case 'm2o_ref': return `m2o → ${transform.model}`
    case 'm2m_ref': return `m2m → ${transform.model}`
    case 'db_id': return `db_id → ${transform.model}`
  }
}

function getTransformClass(transform: FieldTransform, isStandardDbId: boolean): string {
  switch (transform.type) {
    case 'm2o_ref': return 'csv-transform-badge--m2o'
    case 'm2m_ref': return 'csv-transform-badge--m2m'
    case 'db_id': return isStandardDbId ? 'csv-transform-badge--dbid' : 'csv-transform-badge--dbid-warn'
    default: return ''
  }
}
...
async function validateRowForFile(filename: string) {
  ...
  try {
    ...
    for (const [csvHeader, odooField] of Object.entries(mapping.fieldMappings)) {
      ...
      // Match batchExecutor.transformRow logic:
      // Strip /id and /.id suffixes from field names, transform values appropriately

      if (odooField === 'id') {
        // Row-level external ID for upsert
        mappedData['__external_id__'] = value
      } else if (odooField === '.id') {
        // Row-level database ID for upsert
        mappedData['__db_id__'] = parseInt(value, 10)
      } else if (odooField.endsWith('/.id')) {
        // Database ID reference: strip suffix, convert to integer
        const targetField = odooField.slice(0, -4)
        mappedData[targetField] = parseInt(value, 10)
      } else if (odooField.endsWith('/id')) {
        // External ID reference: strip suffix, keep as string for backend resolution
...
async function handleProfileSelect(id: number) {
  ...
  try {
    ...
    // Apply legacy field mappings (csvColumn format)
...
<template>
  <div class="csv-p-6 csv-space-y-6">
    <div class="csv-flex csv-justify-between csv-items-center">
      <h1 class="csv-text-2xl csv-font-semibold">{{ $t('config.title') }}</h1>
      <div class="csv-flex csv-gap-2 csv-items-center">
        <span v-if="activeProfile" class="csv-text-xs csv-text-muted">
    ...
    <Card class="csv-p-4">
      ...
      <div v-if="profiles.loading" class="csv-text-sm csv-text-muted">{{ $t('config.loadingProfiles') }}</div>
      ...
      >
        <option value="">{{ $t('config.noProfileSelected') }}</option>
        <option v-for="p in profiles.profileList" :key="p.id" :value="p.id">
          {{ p.name }} (v{{ p.version }})
        </option>
      </select>
      <div v-if="profileLoading" class="csv-text-sm csv-text-muted csv-mt-1">{{ $t('config.loadingProfileData') }}</div>
    </Card>
              ...
              >
                ...
                <div class="csv-mapping-col csv-mapping-col--transform">
                  <span
                    ...
                    :class="getTransformClass(getMappingInfoForFile(file.name, header)!.transform, getMappingInfoForFile(file.name, header)!.isStandardDbId)"
                  >
                    {{ formatTransform(getMappingInfoForFile(file.name, header)!.transform) }}
                  </span>
