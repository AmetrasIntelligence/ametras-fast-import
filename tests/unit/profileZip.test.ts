...
import { importProfileFromZip, exportProfileToZip } from '@/utils/profileZip'

describe('importProfileFromZip', () => {
  function createZipFile(files: Record<string, string>): File {
    const zip = new JSZip()
    for (const [name, content] of Object.entries(files)) {
      zip.file(name, content)
    }
    // We need to return a mock File - JSZip generates async so we test differently
    return null as any // placeholder - real tests below use async approach
  }
